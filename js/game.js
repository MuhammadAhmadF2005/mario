...
function updateEntities(dt, gameTime) {
  ...
  if (player.exiting) {
    ...
    // Center the camera on Mario when possible
    if (player.pos[0] > vX + 80) {
      vX = player.pos[0] - 80;
    } else if (player.pos[0] < vX + 64 && vX > 0) {
      vX = player.pos[0] - 64;
    }
  }
...
function render() {
  ...
  if (level.backgroundImg) {
    // Parallax scrolling: background moves slower than foreground
    var bgX = (vX / 2.5) % level.backgroundImg.width;

    // Draw the image twice side-by-side to allow seamless looping
    ctx.drawImage(level.backgroundImg.img, -bgX, 0);
    ctx.drawImage(level.backgroundImg.img, -bgX + level.backgroundImg.width, 0);
  } else {
